...
public class ShowInterestShell 
{
    ...
    public static void getInfo(Scanner kb)
    {
      ...
      String phone = String.format("(%s) %s-%s"),
            rawNumber.substring(1, 3);
            rawNumber.substring(4, 6);
            rawNumber.substring(7, 10));
    ...
    public static String getName(Scanner kb)
    {
        ...
        String name = kb.nextLine().toUpperCase();
        return name;
    }
   ...
   {   
    ...
    public static void printTable(double amount, int periods, double rate,double deposit) //period = years , amount = initial amount, rate = interest rate
    {
        //print the header such as Year Interest deposit new balance
        System.out.println("Year\tInterest\tDepositNewBalance");
    ...
    {
      double round2 = Math.round(n * 100.0)/ 100.0;
        return round2;
    }
}
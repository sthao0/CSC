		...
		stats_t *race_stats )             /* in & out */	
{
	...
	for (d=0;d<NRACERS;d++)
	{
		...
		driver_list[d].best_time=driver_list[0].tries[0];
		for(t=0;t<TRIES;t++)
		{
		...
		race_stats->best_average=race_stats->best_average + driver_list[d].best_time;
	}
		...
		stats_t *race_stats )
{
	...
	{
		m=NRACERS/2; 
		race_stats->median=driver_list[m].best_time;
	}
	else
	{
		m=NRACERS/2;
		race_stats->median= (driver_list[m].best_time+driver_list[m-1].best_time);
	}
